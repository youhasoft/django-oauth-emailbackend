...
TASK_CONFIG = {'name':          'asyncmailer_youhasoft', 
               'ignore_result': True,
...
def send_emails(messages, site_id, 
    ...
    # print('tasks:---------site_id-------', site_id)
    # print('tasks:---------email_server_name-------', email_server_name)
    # print('tasks:---------email_server_database-------', email_server_database)
    ...
    for idx in range(0, len(messages)):
        ...
        # by odop 2019.3.20
        # bcc에 보내는 계정을 더해서 보낸 메일함에 들어가지 않는 대신 참조로 발송 내역을 확인할 수 있도록 한다.
        
        # by odop 2019.8.24
        # EmailServer 설정에서 use_bcc가 참인 경우 참조한다.
#         if use_bcc:
#             messages[idx]['bcc'] = settings.BCC_EMAILS + [combined_kwargs['username']]
#         else:
#             messages[idx]['bcc'] = settings.BCC_EMAILS 
